...
| **Aksepterte allokeringer (`eventId` satt)** | 26 (100 %) | 3 (11.5 %) | Kun strenge sikkerhetskrav gir akseptert allokering |
| **Uavklarte / avviste (`eventId: undefined`)** | 0 (0 %) | 23 (88.5 %) | Sikker failure mode for usikre og svake kandidater |
| **Confidence-fordeling:** | | | |
| – High confidence | 26 (100 %) | 3 (11.5 %) | Kun tildelinger med tidskausalt bevis, korroborering og margin |
| – Medium confidence | 0 (0 %) | 10 (38.5 %) | Kandidater med moderat margin / usikre datoer (kun for manuell review) |
...
| **Eksakt korrekte aksepterte allokeringer** | 11 | 3 | Herd #5 (1-0), Herd #9 (2-0), Aksla #7 (3-0) |
...
| **Delvis korrekte grupper** | 2 | 1 | `1963|clausenengen` |
| **Feilede grupper** | 3 | 4 | `1963|raufoss-il`, `1960|langevag`, `1959|skarbovik`, `1964|andalsnes` |
...
### 2. Query-relativ avstemming (Herd 1962 #9 løst)
- `analyzeEvent` i `reconciliation.ts` evaluerer `scoreFound` dynamisk mot `query.expectedScore` for den konkrete hypotesen som avstemmes (med støtte for begge lagrekkefølger).
- **Resultat:** Herd 1962 #9 får `status: confirmed`, `checks.score: confirmed` og ingen falsk kildekonflikt.

### 3. Tidskausalt bevis og streng margin for High Confidence
- En negativ eller null margin kan **aldri** gi medium eller high confidence.
- `confidence: "high"` krever:
  1. Tidskausalt bevis med `dateConfidence: "high"`.
  2. Resultatmatch mot kildens tall (`hasScoreMatch: true`).
  3. Kronologisk forankring (foregående søsken i gruppen er ikke uoppklart, eller eksplisitt konkurransematch).
  4. Sterk hypotese-spesifikk margin (`margin >= 20`) og høy score (`score >= 80`).
- **Resultat:** Falske high-confidence allokeringer redusert fra **8 til 0**.

### 4. Symmetrisk avvisning
...
- **Allokering:** Begge hypoteser forblir uallokert (`confidence: low`, `decision: unresolved`).
...
- **Allokering:** `1948 #10` forblir uallokert (`confidence: low`, `decision: unresolved`).
...
- **Allokering:** Forblir uallokert med `confidence: medium/low`. Ingen falsk high confidence.
...
- **Allokering:** #5 korrekt avvist som uallokert (`correctly_rejected`). #17 forblir uallokert kandidat (`medium`).
...
### 9. `1964|andalsnes` – **FAILED (TRYGT FEILMODUS)**
- **Fasit:** #16 (4–0 vs 6–1 cup, 1964-05-24) er reell kildekonflikt.
- **Allokering:** Forblir uallokert med kandidat 1964-05-24 (`low` margin). Ingen falsk high confidence.
...
- Ingen falsk overkonfidens oppstår for usikre hendelser (Raufoss, Sarpsborg, Kvik, Clausenengen, Aksla #9).
- Kontrollkravene for trygg failure mode er oppfylt.
- Siden flere verifiserte enkelthendelser (Skarbøvik, Langevåg, Åndalsnes) inntil videre holdes uallokerte av de strenge sikkerhetskravene, beholdes status `NEEDS_FURTHER_SIBLING_FIX` inntil ranking- og berikelsestilpasninger er evaluert på et senere tidspunkt.
