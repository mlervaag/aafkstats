...
if (inputIndex < 0 || !args[inputIndex + 1]) throw new Error("Bruk --input <manifest.json>.");
...
const manifest = JSON.parse(await readFile(resolve(invocationDir, args[inputIndex + 1]!), "utf8")) as unknown;
const archive = await loadArchive(dataDir());
const result = generateNewspaperVerificationCases(manifest, archive.verificationCases);
