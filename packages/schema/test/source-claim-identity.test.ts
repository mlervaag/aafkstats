...
describe("Source Claim Stable Identity & Lineage (PR #207)", () => {
  ...
  it("TEST A: Alle 2015 kilderesultater har gyldig, unik 128-bit claimId (32 hex-siffer) i arkivet", () => {
    const allClaims = archive.sourceResults.flatMap(flattenSourceResults);
    expect(allClaims).toHaveLength(2015);
    ...
    expect(claimIds.size).toBe(2015);
  });
  ...
  it("TEST B: Backfill-verktøyet er reelt idempotent ved dobbel --apply i temp-katalog", async () => {
    ...
    try {
      ...
      expect(res1.totalSourceResults).toBe(2015);
      expect(res1.claimIdsCreated).toBe(0);
      expect(res1.claimIdsUnchanged).toBe(2015);
      expect(res1.uniqueClaimIds).toBe(2015);
      ...
      expect(res2.totalSourceResults).toBe(2015);
      expect(res2.claimIdsCreated).toBe(0);
      expect(res2.claimIdsUnchanged).toBe(2015);
      expect(res2.uniqueClaimIds).toBe(2015);
