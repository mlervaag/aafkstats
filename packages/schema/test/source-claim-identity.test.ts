...
describe("Source Claim Stable Identity & Lineage (PR #207)", () => {
  ...
  it("TEST A: Alle 2053 kilderesultater har gyldig, unik 128-bit claimId (32 hex-siffer) i arkivet", () => {
    const allClaims = archive.sourceResults.flatMap(flattenSourceResults);
    expect(allClaims).toHaveLength(2053);
    ...
    expect(claimIds.size).toBe(2053);
  });
  ...
  it("TEST B: Backfill-verktøyet er reelt idempotent ved dobbel --apply i temp-katalog", async () => {
    ...
    try {
      ...
      expect(res1.totalSourceResults).toBe(2053);
      expect(res1.claimIdsCreated).toBe(0);
      expect(res1.claimIdsUnchanged).toBe(2053);
      expect(res1.uniqueClaimIds).toBe(2053);
      ...
      expect(res2.totalSourceResults).toBe(2053);
      expect(res2.claimIdsCreated).toBe(0);
      expect(res2.claimIdsUnchanged).toBe(2053);
      expect(res2.uniqueClaimIds).toBe(2053);
