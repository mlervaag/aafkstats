...
import { ocrAnnualReportPdf, ocrCoverageReport, ocrManifest, type AnnualReportOcrResult } from "../adapters/sfk-annual-report-ocr.js";
...
interface Args { from: number; to: number; refresh: boolean; report?: string }
...
async function main(): Promise<void> {
  ...
  const reports = discovered.filter((report) => report.year >= args.from && report.year <= args.to);
  ...
  const manifestPath = resolve(root, ".cache/ingest/sfk-annual-reports/ocr-manifest-1952-1979.json");
...
function parseArgs(argv: string[]): Args {
  ...
  for (let index = 0; index < argv.length; index++) {
    ...
    if (arg === "--refresh") refresh = true;
      ...
      if (!raw || !Number.isInteger(year)) throw new Error(`${arg} krever et årstall`);
  ...
  if (from < 1952 || to > 1979 || from > to) {
    throw new Error("OCR-spennet i denne kommandoen må ligge innenfor 1952–1979");
  }
  return { from, to, refresh, report };
}
