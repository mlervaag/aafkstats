...
export interface SourcedPlayerProfile {
  providerId: string;
  providerName: string;
  name: string;
  url: string;
...
/**
 * Finner bare en eksakt arkividentitet. En ekstern profil får aldri skape en
...
export function resolvePlayerTarget(archive: Archive, query: string): PlayerTarget {
  ...
  const lineupNames = aafkLineupNames(archive);
  ...
  if (existing) {
    const isPlayer = existing.position !== undefined
      ...
      || [existing.name, ...existing.names].some((name) =>
        lineupNames.some((lineupName) => personKey(lineupName) === personKey(name)));
...
export function planSourcedPlayerProfile(target: PlayerTarget, profile: SourcedPlayerProfile): ProfilePlan {
  const sameName = personKey(profile.name) === personKey(target.name);
  const longerForm = isLongerNameForm(target.name, profile.name) || isLongerNameForm(profile.name, target.name);
  if (!sameName && !longerForm) {
    throw new Error(
      `${profile.providerName}-navnet «${profile.name}» kan ikke knyttes sikkert til «${target.name}». `
  ...
  if (longerForm && ![base.name, ...names].some((name) => personKey(name) === personKey(profile.name))) {
    names.push(profile.name);
    changes.push(`navneform: ${profile.name}`);
  }

  const position = missingOrConflict("posisjon", base.position, profile.position, profile.providerName, changes, conflicts);
  const nationality = missingOrConflict("nasjonalitet", base.nationality, profile.nationality, profile.providerName, changes, conflicts);
  const wikidata = missingOrConflict("Wikidata", base.wikidata, profile.wikidata, profile.providerName, changes, conflicts);
  ...
  const providers = sourceFields.length === 0 ? base.providers : [
    ...base.providers.filter((provider) => provider.url !== profile.url),
    {
      ...
      url: profile.url,
      ...(profile.retrievedAt ? { retrievedAt: profile.retrievedAt } : {}),
      fields: sourceFields,
      note: profile.note,
    },
  ];
...
function missingOrConflict<T extends string>(
  ...
  if (personKey(existing) !== personKey(incoming)) {
    conflicts.push(`${label}: arkivet har «${existing}», ${providerName} viser «${incoming}»`);
  }
  return existing;
}
