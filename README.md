...
| **1777 kildedokumenterte resultatoppføringer** | Resultatobservasjoner bevart direkte fra historiske kilder; selve oppføringene teller ikke som kamper. **1600 mangler fortsatt kobling til en kanonisk kamp.** |
...
| `pnpm ingest:nb-newspaper-discover -- --source-result FIL [--year ÅR] [--no N] [--unlinked-only]` | Avisdiscovery for kildeførte resultater: dato fra tidsuttrykk, kontroll av resultat og hjemme/borte, og eksplisitt konflikt der kildene er uenige. Se [NB_AVISDISCOVERY.md](docs/NB_AVISDISCOVERY.md). Skriver aldri data |
