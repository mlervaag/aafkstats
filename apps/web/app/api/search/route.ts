import { searchMatches, searchPeople } from "@/lib/search";
...
export function GET(request: Request): Response {
  ...
  if (query.length < 2) return Response.json({ matches: [], people: [] });
  if (query.length > 100) return Response.json({ error: "Søket er for langt." }, { status: 400 });

  try {
    return Response.json({ matches: searchMatches(query), people: searchPeople(query) });
  } catch (error) {
