...
export const openApiDocument = {
  ...
  paths: {
    "/meta": { get: { summary: "Versjon, rettigheter og dekning", responses: { "200": ok({ type: "object" }) } } },
    "/results": { get: { summary: "Hele resultathistorien med evidensnivå", description: "Returnerer canonical_match og grupperte source_claim-rader separat. Ikke summer evidensnivåene.", parameters: [integer("season"), integer("seasonFrom"), integer("seasonTo"), text("opponent"), text("opponentClubId"), text("result", ["S", "U", "T"]), text("ranking", ["largest_win", "largest_defeat", "most_goals_for", "most_goals_total", "newest", "oldest"]), integer("limit", "1–100, standard 20")], responses: { "200": ok({ type: "array", items: { $ref: "#/components/schemas/Result" } }), "400": error } } },
    "/matches": { get: { summary: "Kanoniske kamper", parameters: [integer("season"), integer("seasonFrom"), integer("seasonTo"), text("opponent"), text("competitionType"), text("result", ["S", "U", "T"]), integer("limit", "1–100, standard 20")], responses: { "200": ok({ type: "array", items: { type: "object" } }), "400": error } } },
    "/matches/{id}": { get: { summary: "Én kanonisk kamp", parameters: [{ name: "id", in: "path", required: true, schema: { type: "string" } }], responses: { "200": ok({ type: "object" }), "404": { description: "Ikke funnet" } } } },
    "/seasons/{year}": { get: { summary: "Sesongsammendrag og dekning", parameters: [{ name: "year", in: "path", required: true, schema: { type: "integer" } }], responses: { "200": ok({ type: "array", items: { type: "object" } }), "404": { description: "Ikke funnet" } } } },
    "/research/overview": { get: { summary: "Aggregert oversikt over mangler", responses: { "200": ok({ type: "object" }) } } },
    "/research/cases": { get: { summary: "Bare publiserte, åpne verification cases", parameters: [text("category"), text("targetType"), integer("priority"), integer("limit", "1–100, standard 20")], responses: { "200": ok({ type: "array", items: { $ref: "#/components/schemas/VerificationCase" } }), "400": error } } },
    "/research/cases/{id}": { get: { summary: "Én publisert, åpen verification case", parameters: [{ name: "id", in: "path", required: true, schema: { type: "string" } }], responses: { "200": ok({ $ref: "#/components/schemas/VerificationCase" }), "404": { description: "Draft, paused, resolved eller ukjent sak" } } } },
    "/openapi.json": { get: { summary: "Denne OpenAPI-kontrakten", responses: { "200": { description: "OpenAPI 3.1" } } } },
  },
  components: { schemas: {
