...
export const openApiDocument = {
  ...
  components: { schemas: {
    ...
    Result: {
      type: "object",
      required: ["evidenceLevel", "recordId", "season", "missingFields", "sources"],
      properties: {
        evidenceLevel: { type: "string", enum: ["canonical_match", "source_claim"] },
        recordId: { type: "string" },
        resultGroupId: { type: ["string", "null"] },
        date: { type: ["string", "null"] },
        datePrecision: { type: ["string", "null"] },
        season: { type: "integer" },
        opponent: { type: ["string", "null"] },
        opponentClubId: { type: ["string", "null"] },
        aafkScore: { type: ["integer", "null"] },
        opponentScore: { type: ["integer", "null"] },
        goalDifference: { type: ["integer", "null"] },
        result: { type: ["string", "null"], enum: ["S", "U", "T", null] },
        competitionId: { type: ["string", "null"], description: "Arkivets konkurranse-ID." },
        competition: { type: ["string", "null"], description: "Konkurransens visningsnavn. Null når ID-en ikke finnes i konkurranseregisteret." },
        confidence: { type: ["string", "null"], description: "Hvor godt den kanoniske oppføringen er dokumentert, ikke en sannsynlighet for at resultatet er riktig." },
        completeness: { type: ["number", "null"], description: "Andel utfylte kampfelt mellom 0 og 1. Dekningsgrad, ikke sannsynlighet." },
        hasConflicts: { type: "integer" },
        sourceCount: { type: ["integer", "null"], description: "Antall unike kilder for en gruppert source_claim." },
        note: { type: ["string", "null"], description: "Kildenotatene satt sammen til én lesbar tekst." },
        notes: { type: "array", description: "Ett element per kildenotat. Notatene kommer fra ulike kilder.", items: { type: "string" } },
        claimSummary: { type: ["object", "null"], description: "Hvor enige kildene i gruppen er med hverandre. Ikke et mål på om påstanden er sann." },
        claims: { type: ["array", "null"], description: "Originale kildepåstander, inkludert motstandernavn slik kilden trykte det.", items: { type: "object" } },
